      ...
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item title="商品SKU信息" name="skuInfo">
          ...
          <el-form-item label="商品SKU">
            <el-table :data="product.productSkuList" style="width: 100%">
...
import { FindCategoryByParentId } from '@/api/product/category.js'
import {
  GetProductPageList,
  SaveProduct,
  UpdateProductById,
...
import { FindBrandByCategoryId } from '@/api/product/categoryBrand.js'
import { FindAllProductUnit } from '@/api/product/productUnit.js'
...
const saveData = async () => {
  let { data, message } = await SaveProduct(product.value)
  if (data) {
...
const updateData = async () => {
  let { data, message } = await UpdateProductById(product.value)
  if (data) {
    dialogVisible.value = false
    ElMessage.success('操作成功')
    fetchData()
